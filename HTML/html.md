...
div、p、h1~h6、ul、ol、dl、li、dd、table、hr、blockquote、address、table、menu、pre，HTML5新增的header、section、aside、footer等都是块元素
...
常见的内联元素：
a img iframe span、img、a、lable、input、abbr（缩写）、em（强调）、big、cite（引用）、i（斜体）、q（短引用）、textarea、select、small、sub、sup，strong、u（下划线）、button（默认display：inline-block等等

span没有任何的语义，专门用来选中文字，然后为文字设置样式

内联元素主要用来用来选中文字，然后为文字设置样式

一般情况下只使用块元素去包含内联元素，而不会使用一个内联元素去包含一个块元素
a可以包含任意元素，出了它本身
p元素不可以包含任何的块元素
